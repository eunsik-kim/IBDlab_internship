...
from model import SurveyClassifier
from utils import get_logger, drop_top_n_questions, plot_heatmap, top_n_importance

# It may need to be set differently depending on GPU memory usage by model type.
# 'sentence-transformers/paraphrase-multilingual-mpnet-base-v2' 768
# 'sentence-transformers/xlm-r-100langs-bert-base-nli-stsb-mean-tokens' 768 
# 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2' 384 118145667 12 layer 
# 'mental/mental-bert-base-uncased' 768, 109974147 12 layer
...
def main(arg):
    ...
    # model
    if arg.model_name == 'mnaylor/psychbert-cased':
        bert_model = AutoModel.from_pretrained(arg.model_name, from_flax = True)
    else:
        bert_model = AutoModel.from_pretrained(arg.model_name)
    bert_hidden_dim = bert_model.embeddings.word_embeddings.weight.shape[-1]
    total_query_num = 427
    label_nums = 4
    label_word = ['normal', 'think', 'plan', 'try']
    model = SurveyClassifier(bert_model, hidden_dims=[total_query_num * bert_hidden_dim, label_nums], avg = arg.avg_mode, max = arg.max_mode, cls = arg.cls_mode, ast = arg.ast_mode) # avg = True, max = False, cls = False, ast = False
    ...
    for name, param in model.named_parameters():
        ignore_weight = ['embeddings'] if arg.cls_mode else ['embeddings', 'pooler']
        if name.split('.')[1] in ignore_weight:  
            param.requires_grad = False
            logger.info(f"Parameter {name} has been freezed")
        elif name.split('.')[1] == 'encoder':
            if int(name.split('.')[3]) <= 4:
                param.requires_grad = False
                logger.info(f"Parameter {name} has been freezed")
            else:
                logger.info(f"Parameter {name} wiil be tuned")    
        else:
            logger.info(f"Parameter {name} wiil be tuned")
    ...
    # load latest chckpoint
    start_epoch = 1
    '''
    if os.path.isdir(arg.data_dir):
        dirlist = os.listdir(arg.data_dir)
        if 'logs' in dirlist:
            dirlist.remove('logs')
        if dirlist: 
            latest_file = sorted(dirlist)[-1]
            ckpt = torch.load(os.path.join(arg.data_dir, latest_file))
            model.load_state_dict(ckpt['model'])
            optimizer.load_state_dict(ckpt['optimizer'])
            scheduler.load_state_dict(ckpt['scheduler'])
            start_epoch = ckpt['epoch']
            best_accuracy, best_epoch = ckpt['best_acc'], ckpt['best_epoch']
            logger.info(f'{start_epoch}epoch-checkpoint load compeletely')
    '''
    num_epochs = 30
